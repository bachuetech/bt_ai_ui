...
async function submitRequest() {
  ...
  var selectedModel = "";
  var full_model_to_request = "";
  if (use_default_ai_data_only) {
    selectedModel = default_ai_model; //DEFAULT WITH JEREMY!
    full_model_to_request = default_ai_platform + ":" + default_ai_model + ":" + default_ai_model_version
  } else {
    selectedModel = getSelectedModel(); //NO NEED WITH JEREMY!
    full_model_to_request = selectedModel;
  }

  var model_ctx = retrieveContext(selectedModel);
  data = { current_time: getCurrentTime(), current_date: getCurrentDate(), model: full_model_to_request, prompt: input }
  if (model_ctx) {
    data = { current_time: getCurrentTime(), current_date: getCurrentDate(), model: full_model_to_request, prompt: input, context: model_ctx }
  }
  ...
  postRequest(chat_URL, data, interrupt.signal)
    .then(async response => {
      if (!response.ok) {
        showNotiMsg("Communication Error", "Network response error", response.statusText + " (" + response.status + ")");
...
function getSelectedModel() {
  if (use_default_ai_data_only) {
    return default_ai_model; //DEFAULT WITH JEREMY!
  } else {
